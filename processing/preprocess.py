...
def read_csv_to_df():
    # --- FIXED PATHS: Reading both the csv files from the root directory (where they are downloaded) ---
    credit_ = pd.read_csv(r'tmdb_5000_credits.csv')
...
def get_details(selected_movie_name):
    ...
    # --- FIXED PATHS: Reading PKL files from the root directory ---
    pickle_file_path = r'movies_dict.pkl'
    with open(pickle_file_path, 'rb') as pickle_file:
        loaded_dict = pickle.load(pickle_file)

    movies = pd.DataFrame.from_dict(loaded_dict)

    pickle_file_path = r'movies2_dict.pkl'
    with open(pickle_file_path, 'rb') as pickle_file:
